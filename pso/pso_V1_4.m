function [book,Pbest] = pso_V1_4(N,D,x,v,w)
...
for i=1:N                                 %第一位 读者 从高处走来 去寻找查拉图斯特拉的灵魂
    p(i)=fitness3(x(i,:),D,w(i,:));               %这时他在林中遇到一位老者，老者说 "上帝已死，没有道德和真理，没有人可以成为至人"
...
for i=2:N                                 %其后的每一位 读者 从高处走来 去寻找查拉图斯特拉的灵魂
    if fitness3(x(i,:),D,w(i,:))<fitness3(pg,D,w(i,:))    %读者们都深知自己无法得到真理的全貌，但是可以从自己的一生中获得经验
...
for t=1:MaxDT                            %新生的查拉图斯特拉睁开了双眼
    for i=1:N                            %此刻的他 既不是 读者 也不是 查拉图斯特拉
        ...
        if fitness3(x(i,:),D,w(i,:))<p(i)        %他说到 如果真理不是我所定义和创造的"
            p(i)=fitness3(x(i,:),D,w(i,:));      %那么神所创造的 虚伪、无聊的真理 不要也罢"
            y(i,:)=x(i,:);               %“我就是我的真理”
        end                              %“任凭是神也无法撼动我的存在”
        if p(i)<fitness3(pg,D,w(i,:))            %然而 查拉图斯特拉 并不知道 上帝并没有死
    ...
    Pbest(t)=fitness3(pg,D,w(i,:));              %他掌握了人性的真实
    book{t}=story;                       %文明汇聚成册
